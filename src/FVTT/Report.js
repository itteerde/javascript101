const width = 650;
const SHOW_HIT_DICE = true;
const SHOW_LANGUAGES = true;
const SHOW_TREASURY = true;
const TREASURY_UUID = "Actor.8AEpgEDR8sabumTi.Item.5BeqQFcPUAb4R9bX";
const TREASURY_RESERVES = "Actor.8AEpgEDR8sabumTi.Item.5OiWXgT8xxdv7w1r";
...
function hit_dice(party) {
    let html = '';
    party.forEach(a => {
        let hit_dice_string = '';
        let classes = Object.getOwnPropertyNames(a.classes);
        classes.forEach(c => hit_dice_string += (a.classes[c].system.hd.value + a.classes[c].system.hd.denomination + ','));
        hit_dice_string = hit_dice_string.slice(0, -1);
        html += `<td align="center">${hit_dice_string}</td>`;
    });
    return html;
}
...
content += `
    <table>
        ...
        ${SHOW_HIT_DICE ? '<tr><td>Hit Dice</td>' + hit_dice(party) + '</tr>' : ''}
...
<ol>
<li>
Umbratica: @UUID[Actor.8AEpgEDR8sabumTi.Item.TdgL3B1gSzTbj0Vj]{Bag of Holding}
</li>
<li>Glafourra: Spell Book</li>
<li>Glafourra: Spell Book</li>
<li>Glafourra: Spell Book</li>
<li>Katar: Shield if the Hidden Lord</li>
<li>Evren: Mace?</li>
<li>Rumil: @UUID[Actor.4L8PrR4joDa9LaY9.Item.rfL8dCrTCFT7gWOW]{Puzzle Box}</li>
<li>Thistle: Battleaxe +1</li>
</ol>
</div>
                `})
        }
